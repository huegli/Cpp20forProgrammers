...
int main() {
    ...
    // match any charactor that's not a lowercase letter
    std::regex r4{"[^a-z]"};
    std::cout << "Matching against: [^a-z]\n"
        << fmt::format("A: {}; a: {}\n\n",
                std::regex_match("A", r4), std::regex_match("a", r4));

    // match metacharactors as literals in a custom charactor class
    std::regex r5{"[*+$]"};
    ...
    std::cout << "Matching against: [A-Z][a-z]+\n"
        << fmt::format("Wally: {}; t: {}\n\n",
                std::regex_match("Wally", r6), std::regex_match("t", r6));
    ...
    // matching n (3) or more occurences of a subexperssion
    std::regex r8{R"(\d{3,})"};
