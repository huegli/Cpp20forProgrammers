...
MyArray::MyArray(size_t size)
   ...
   std::cout << "MyArray(size t) constructor\n";
}
...
MyArray::MyArray(std::initializer_list<int> list)
   ...
   std::cout << "MyArray(initializer list) constructor\n";

   // copy list argument's elements into m_ptr's underlying int_array
...
MyArray& MyArray::operator=(MyArray&& right) noexcept {
   ...
   if (this != &right) { // avoid self-assignments
...
std::string MyArray::toString() const {
   ...
   // insert each item in the dynamic array into ostringstream
